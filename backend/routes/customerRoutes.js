...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    const emailCheckQuery = 'SELECT customerID, email FROM customers WHERE LOWER(email) = LOWER($1)';
    console.log(`Executing query: ${emailCheckQuery} with params: [${email.trim()}]`);
    ...
    console.log(`Generated verification code for ${userEmail}: ${verificationCode}`);
...
router.post('/verify-code', async (req, res) => {
  try {
    ...
    if (verification.code !== code) {
      console.log(`Invalid code provided. Expected: ${verification.code}, Got: ${code}`);
      return res.status(400).json({
        success: false,
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    console.log(`PASSWORD RESET SUCCESSFUL: Password reset for ${updateResult.rows[0].customername} (${email})`);
